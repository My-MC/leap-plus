...
// 設定をローカルストレージに保存する関数
function saveSettings(startIndex, endIndex, headerOnly, timeLimit, questionType) {
    const settings = {
...
document.getElementById('wordbook').addEventListener('click', function() {
    ...
    wordDataPromise.then(wordData => {
        ...
        const registeredWordsContainer = document.querySelector('.registered_words');
        if(!opened){
            opened = true;
            wordbook.forEach(word => {
                ...
                registerButton.style.backgroundColor = isRegistered ? '#b72d32' : '#309b61'
                registerButton.textContent = isRegistered ? '登録済み' : '単語帳に登録';
        ...
        document.querySelector('#wordbook_play').addEventListener('click', function() {
...
document.querySelector('#leap').addEventListener('click', function() {
        ...
        wordDataPromise.then(wordData => {
            ...
            wordDataInRange = wordData.filter(word => parseInt(word.id) >= parseInt(startIndex) && parseInt(word.id) <= parseInt(endIndex));
...
document.getElementById('confirm').addEventListener('click', function() {
    ...
    if(parseInt(startIndex) <= 1935 && parseInt(endIndex) <= 1935 && parseInt(endIndex) > parseInt(startIndex)){
        if(parseInt(leapStartIndex) <= 1935 && parseInt(leapLastIndex) <= 1935 && parseInt(leapLastIndex) > parseInt(leapStartIndex)){
    ...
    wordDataPromise.then(wordData => {
        ...
        function nextQuestion() {
            ...
            if (currentQuestionIndex === totalQuestions) {
                    ...
                    const currentRound = parseInt(localStorage.getItem('currentRound') || '0') + 1;
            ...
            let correctMeaning = currentQuestion.definitions[Math.floor(Math.random() * currentQuestion.definitions.length)];
            ...
            if(!playMode){
                questionWord = currentQuestion.definitions[Math.floor(Math.random() * currentQuestion.definitions.length)] ;
            ...
            if(timeLimit){
                ...
                timer = setTimeout(function() {
                    ...
                    const isRegistered = wordbook.some(w => w.id === currentQuestion.id);
                    if(!isRegistered){
            ...
            while (incorrectMeanings.length < 3) {
                ...
                let randomMeaning = randomWord.definitions[Math.floor(Math.random() * randomWord.definitions.length)];

                if(!playMode){
                    randomMeaning = randomWord.word;
                }

                if (!incorrectMeanings.includes(randomMeaning) && randomMeaning !== correctMeaning) {
                    incorrectMeanings.push(randomMeaning);
                }
            }

            // 正解と不正解の意味を結合して選択肢を作成
            const choices = [correctMeaning, ...incorrectMeanings].sort(() => 0.5 - Math.random());
            ...
            choices.forEach(choice => {
                ...
                choiceButton.addEventListener('click', function() {
                    ...
                    // 正解かどうかに応じてメッセージを表示
                    const resultMessage = isCorrect ? '正解！' : `不正解... <br> 正解： ${correctMeaning}`;
                    const resultDiv = document.createElement('div');
                    resultDiv.classList.add('result');
                    resultDiv.innerHTML = resultMessage;
                    questionContainer.prepend(resultDiv);
